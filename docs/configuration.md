...
| `features/overlay/overlay.json` | `overlay` feature | which event fills which field on screen, death-counter texts, and (under `chat`) which platforms, whether `!` lines are hidden, how much history a reconnect gets |
