"""The listener the chat panel browser source hangs on.

The same reversed direction as the OBS relay and the overlay, and for the same reason: the
browser source runs in OBS on the streamer's machine, the bot on a server. So it does not
*fetch* chat but dials in and gets it sent - a message is in the picture the moment it
passes moderation, not at the next poll.

The token check is deliberately the same shape as platforms/obs/link.py and
features/overlay/server.py: same header, same query parameter, same compare_digest. Three
small copies of it are cheaper than a shared abstraction none of them may ever need to
diverge from - see features/overlay/server.py for why the query parameter exists at all
(the browser's WebSocket API cannot set headers of its own).

This file knows only connections and JSON frames. What goes into them is decided by
features/chat_panel/feature.py.
"""
...
class ChatPanelServer:
    """WebSocket server for the chat panel. Holds the open connections and sends each of
    them the same thing: the recent history on connect, one frame per accepted message
    afterwards."""

    def __init__(self, token, bind="0.0.0.0", port=4458, history=None, on_error=None):
        self._token = token
        self._bind = bind
        self._port = port
        # Callback without arguments returning the recent messages as a list - asked on
        # every new connection, so a browser source reloading mid-stream sees the same
        # backlog as one that was there from the start.
    ...
    async def _session(self, connection):
        """One browser source: send the backlog, then hold the connection open. Several may
    ...
    async def broadcast(self, type_, data):
        ...
        swallowed: a panel that happens to be reloading is not the bot's problem."""
        ...
        for result in results:
            if isinstance(result, Exception) and not isinstance(result, websockets.ConnectionClosed):
                self._on_error(f"chat panel broadcast: {result}")
